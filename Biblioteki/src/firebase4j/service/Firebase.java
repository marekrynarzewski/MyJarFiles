...
public class Firebase
{
	...
	public FirebaseResponse get(String path) throws FirebaseException
	{
		...
		FirebaseResponse response = this.processResponse(
				FirebaseRestMethod.GET, httpResponse);
			...
			throws JacksonUtilityException, FirebaseException
	{
		...
		// process the response
		FirebaseResponse response = this.processResponse(
				FirebaseRestMethod.PUT, httpResponse);
	 ...
	 * @throws {@link FirebaseException}
	 */
	public FirebaseResponse put(String path, String jsonData)
			...
			throws JacksonUtilityException, FirebaseException
	{
		...
		// process the response
		FirebaseResponse response = this.processResponse(
				FirebaseRestMethod.POST, httpResponse);
	 ...
	 * @throws {@link FirebaseException}
	 */
	public FirebaseResponse post(String path, String jsonData)
			throws FirebaseException
	{
	...
	public FirebaseResponse delete(String path) throws FirebaseException
	{
		...
		// process the response
		FirebaseResponse response = this.processResponse(
				FirebaseRestMethod.DELETE, httpResponse);
	...
	private StringEntity buildEntityFromJsonData(String jsonData)
			throws FirebaseException
	{
		...
		catch (Throwable t)
		{
			...
			String msg = "unable to create entity from data; data was: "
					+ jsonData;
			LOGGER.error(msg);
	...
	private String buildFullUrlFromRelativePath(String path)
	{
		...
		LOGGER.info("built full url to '" + url + "' using relative-path of '"
				+ path + "'");
			...
			throws FirebaseException
	{
		...
		catch (Throwable t)
		{
			...
			String msg = "unable to receive response from request("
	...
	private FirebaseResponse processResponse(FirebaseRestMethod method,
			HttpResponse httpResponse) throws FirebaseException
	{
		...
		switch (method)
		{
			case DELETE:
				if (httpResponse.getStatusLine().getStatusCode() == 204
						&& "No Content".equalsIgnoreCase(httpResponse
			...
			case GET:
				if (httpResponse.getStatusLine().getStatusCode() == 200
						&& "OK".equalsIgnoreCase(httpResponse.getStatusLine()
								.getReasonPhrase()))
				{
					success = true;
				}
		...
		if (entity != null)
		{
			...
			catch (Throwable t)
			{
				...
				String msg = "unable to read response-content; read up to this point: '"
						+ writer.toString() + "'";
		...
		catch (JacksonUtilityException jue)
		{
			...
			String msg = "unable to convert response-body into map; response-body was: '"
					+ writer.toString() + "'";
			LOGGER.error(msg);
